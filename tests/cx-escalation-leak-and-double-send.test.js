...
test('CX-2: markInboundMessagesAnswered is invoked at the single send-success call site', () => {
  // Duplicate similarity no longer creates an early-return silence: a newer
  // customer turn must receive an answer. The remaining B1 success path marks
  // the inbound answered BEFORE enqueueing the outbound
  ...
  // Any count other than 1 signals either a missing guard or a reintroduced
  // duplicate-suppression path that strands the customer.
  const matches = aiWorkerSource.match(/await markInboundMessagesAnswered\(\{/g) || [];
  assert.equal(matches.length, 1, `expected exactly 1 markInboundMessagesAnswered success-path call site, found ${matches.length}`);
});
