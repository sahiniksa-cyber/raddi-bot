...
const termsHtml = fs.readFileSync(path.join(__dirname, '..', 'dashboard', 'terms.html'), 'utf8');
...
test('server serves a public /privacy page and /data-deletion', () => {
  ...
  assert.ok(serverSrc.includes("app.get('/terms'"), '/terms route missing');
});
...
test('terms page identifies the service and links to privacy policy', () => {
  assert.match(termsHtml, /شروط الخدمة/);
  assert.match(termsHtml, /https:\/\/jwap\.net\/privacy/);
  assert.match(termsHtml, /support@jwap\.net/);
});
