...
| 9 اختبارات سلوكية + CI | ✅ (شريحة عالية القيمة؛ التحويل الشامل مستمر) | `c110ab4` | اختبار auth سلوكي حقيقي + `timeout-minutes` للـ CI + السويت 1505/1505. (تثبيت Node 20.18.1 و`--test-timeout` تراجعنا عنهما لأنهما كسرا CI على تلك النسخة — البوابة كشفتهما فوراً) |
| 10 الأمان/العزل | ✅ (شرائح آمنة متعددة + بنود مؤجَّلة بمبرّر) | `a512651`,`32b7ce9`,`7912bb2`,`de38eca` | نطاقات SSRF + **تثبيت IP ضد DNS rebinding** + **CSRF صارم opt-in** + **circuit breaker لكل تاجر** + 19 اختباراً + السويت 1524/1524 |

## بنود المرحلة 10 المؤجَّلة (بمبرّر تقني — مسموح بشروط القبول)
> **أُنجز منها بأمان:** نطاقات SSRF محجوزة (`a512651`)، تثبيت IP ضد DNS rebinding في fetchURL (`32b7ce9`)، CSRF صارم opt-in (`7912bb2`)، ووحدة circuit breaker لكل تاجر (`de38eca`).

المتبقّي فعلاً يحتاج بيئة staging/حيّة (ممنوعة هنا)؛ نشرها أعمى يخالف شرط "بدون مشاكل":
- **ربط الـ circuit breaker في مسار مزوّد الذكاء:** الوحدة جاهزة ومختبَرة، لكن الربط يمس مسار AI الساخن ويحتاج اختبار حمل.
- **DNS-pinning لمسار safeFetch (undici):** يحتاج dispatcher مخصّص + تحقق شبكي (مسار scan-store الحيّ يستخدم fetchURL المُثبَّت أصلاً).
- **كلمة مرور الأدمن المشتركة → نموذج role/MFA + تقصير الجلسة:** يمس دخول الأدمن؛ يحتاج migration وتحقق حيّ حتى لا يُكسر الدخول.
- **اختبارات الضغط/الحمل/التكامل الحقيقية:** تحتاج Postgres/Redis حقيقيين (غير متوفّرين محلياً).

### السابق (للمرجع):
- **DNS rebinding (تثبيت IP):** حلّ الـ host مرة واحدة والاتصال بالـ IP المُتحقَّق (agent/lookup مخصّص) في `lib/helpers.js` + `ssrf-guard.safeFetch`. يمس ميزة scan-store الحيّة؛ يحتاج تحقق شبكي.
- **كلمة مرور الأدمن المشتركة → نموذج role='admin'/adminEmails + MFA + تقصير الجلسة:** يمس تسجيل دخول الأدمن؛ يحتاج migration وتحقق حيّ حتى لا يُكسر الدخول الحالي.
- **CSRF default-deny لكل مسارات التعديل:** خطر كسر تدفقات مشروعة؛ يحتاج تحقق حيّ للتفريق بين الطلبات المشروعة والـ webhooks.
- **عزل كل تاجر (circuit breaker + حدود لكل تاجر):** تغيير معماري في مسار العامل الساخن؛ يحتاج اختبار حمل/تكامل.

**ملاحظة:** ثغرة SSRF IPv6-mapped أُغلقت فعلاً (فرع reliability-system)، وطبقة منع-التكرار في الجلب (redirect re-validation + الآن نطاقات محجوزة أوسع) قائمة.
