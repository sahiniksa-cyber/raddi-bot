...
class ConnectionManager {
  ...
  constructor({ logger, heartbeat, sessionPath, dataDir, isOtherBotRunning }) {
    ...
    this.allowChromeKill = process.env.KILL_CHROME_ON_RESTART === 'true';
  ...
  start(retryCount = 0) {
    ...
    // Optional deep cleanup. Disabled by default because killing Chrome can drop healthy sessions.
    if (this.allowChromeKill && !this.isOtherBotRunning()) {
