...
// Returns a dns.lookup-compatible function that always resolves to a
// pre-validated address, defeating DNS rebinding (TOCTOU) between validation and
// connect. The Host header and TLS SNI still use the original hostname.
function pinnedLookup(addresses) {
  return (hostname, options, callback) => {
    ...
    try {
      if (opts.all) {
        return cb(null, addresses.map((a) => ({ address: a, family: net.isIP(a) || 4 })));
      }
      return cb(null, addresses[0], net.isIP(addresses[0]) || 4);
    } catch (err) {
      return cb(err);
    }
  };
...
async function fetchURL(url, extraHeaders = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      timeout: 25000,
      // SSRF anti-rebinding: connect to the exact IP assertPublicUrl validated,
      // so a DNS answer that flips to a private/internal IP on this second
      // resolution can't be reached. Falls back to normal DNS if none pinned.
      ...(Array.isArray(addresses) && addresses.length ? { lookup: pinnedLookup(addresses) } : {}),
    };
