...
function stripPhrase(text, phrase) {
  const normalizedPhrase = normalizeArabic(phrase).toLowerCase().trim();
  if (!normalizedPhrase) return text;

  const tokens = normalizedPhrase.split(/\s+/).filter(Boolean);
  const containsWordChars = /[؀-ۿA-Za-z0-9]/.test(normalizedPhrase);
  const matcher = tokens.length > 1
    ? new RegExp(tokens.map(escapeRegExp).join('[\\s،,؛:.!?؟]+'), 'g')
    : new RegExp(escapeRegExp(normalizedPhrase), 'g');
  const boundaryAware = tokens.length === 1 && containsWordChars;
  const lines = text.split('\n');
  return lines.map(line => {
    let working = line;
    let workingNormalized = normalizeArabic(line).toLowerCase();
    let from = 0;
    while (from <= workingNormalized.length) {
      matcher.lastIndex = from;
      const match = matcher.exec(workingNormalized);
      if (!match) break;
      const idx = match.index;
      const before = workingNormalized[idx - 1];
      const after = workingNormalized[idx + match[0].length];
      if (normalizedPhrase === '.' && /\d/.test(before || '') && /\d/.test(after || '')) {
      ...
      working = working.slice(0, idx) + working.slice(idx + match[0].length);
      workingNormalized = workingNormalized.slice(0, idx) + workingNormalized.slice(idx + match[0].length);
      from = idx;
    }
    return working;
  }).join('\n');
...
const PLATFORM_IDENTITY_AVOID_LIST = [
  'ذكاء اصطناعي',
  'AI',
  'OpenAI',
  'ChatGPT',
...
const STRUCTURAL_IDENTITY_DISCLOSURE_RES = [
  /(^|[^\p{L}\p{N}])(?:[إأآا]نا|[إأآا]ني|بصفتي|كوني|ك)[\s،,؛:.!?؟]*(?:مجرد[\s،,؛:.!?؟]*)?(?:(?:مساعد|نظام|برنامج)[\s،,؛:.!?؟]*)?(?:ذكاء[\s،,؛:.!?؟]+اصطناعي|[آا]لي|بوت|روبوت|نموذج[\s،,؛:.!?؟]+لغ[ةه]|chatgpt|chatbot|شات[\s،,؛:.!?؟]*جي[\s،,؛:.!?؟]*بي[\s،,؛:.!?؟]*تي)(?=$|[^\p{L}\p{N}])/giu,
  /(^|[^\p{L}\p{N}])(?:i\s*(?:am|m|['’]m)|as)\s+(?:an?\s+)?(?:(?:a[\W_]*i|artificial\s+intelligence)(?:[\s.,;:!?-]+(?:assistant|system|program))?|bot|chatbot|language\s+model)(?=$|[^\p{L}\p{N}])/giu,
];

function stripStructuralIdentityDisclosure(text) {
  const input = String(text || '');
  const cleaned = STRUCTURAL_IDENTITY_DISCLOSURE_RES.reduce(
    (out, matcher) => out.replace(matcher, (_, prefix) => prefix || ''),
    input,
  );
  if (cleaned === input) return { text: input, removed: false };
  return {
    text: cleaned
      .replace(/^\s*[،,؛:!؟.]+\s*/, '')
      .replace(/^(?:لكن|ولكن)\s+/i, '')
      .replace(/\s*[،,]\s*(?:لكن|ولكن)\s+/gi, '، ')
      .trim(),
    removed: true,
  };
}

function effectiveAvoidList(config = {}) {
  const replyStyle = config?.replyStyle || {};
  const platformDefaults = replyStyle.allowAutomationDisclosure === true
    ? []
    : PLATFORM_IDENTITY_AVOID_LIST;
  const configured = [
    ...(Array.isArray(replyStyle.avoidWords) ? replyStyle.avoidWords : []),
    ...(Array.isArray(replyStyle.avoidPhrases) ? replyStyle.avoidPhrases : []),
  ];
  const seen = new Set();
  return [...platformDefaults, ...configured]
    .filter(value => typeof value === 'string' && value.trim())
    .map(value => value.trim())
    .filter(value => {
      const key = normalizeArabic(value).toLowerCase().replace(/\s+/g, ' ');
...
function stripUnauthorizedEmployeePersona(text, config = {}) {
  const replyStyle = config?.replyStyle || {};
  const employeeName = String(replyStyle.employeeName || '').trim();
  if (!employeeName || replyStyle.employeeNameEnabled === true) {
    return { text: String(text || ''), removed: false };
  }
  const matcher = new RegExp(
    `(?<![\\p{L}\\p{N}])(?:[إأآا]نا|[إأآا]ني|معك|معاكم)\\s+${escapeRegExp(employeeName)}(?=$|[^\\p{L}\\p{N}])`,
    'giu',
  );
  const input = String(text || '');
  const cleaned = input.replace(matcher, '');
  return { text: cleaned, removed: cleaned !== input };
}

function scanForbiddenContent(reply, config = {}) {
  const input = String(reply || '');
  const matches = [];
  for (const phrase of effectiveAvoidList(config)) {
    if (stripPhrase(input, phrase) !== input) matches.push(phrase);
  }
  if (config?.replyStyle?.allowAutomationDisclosure !== true
    && stripStructuralIdentityDisclosure(input).removed) {
    matches.push('automation_identity_disclosure');
  }
  if (stripUnauthorizedEmployeePersona(input, config).removed) {
    matches.push('unauthorized_employee_persona');
  }
  return {
...
function protectEscalationMarkers(text) {
  const markers = [];
  const masked = String(text || '').replace(/\[تحويل:[^\]]*\]/g, (marker) => {
    markers.push(marker);
...
function restoreEscalationMarkers(text, markers) {
  return String(text || '').replace(/\uE000(\d+)\uE001/g, (match, index) => (
    markers[Number(index)] !== undefined ? markers[Number(index)] : match
  ));
}

function replacePrivateContactNames(text, markers = []) {
  const genericLabels = new Set(['الفريق', 'الدعم', 'الموظف', 'المختص']);
  const names = markers.flatMap((marker) => {
    const selected = String(marker || '').match(/^\[تحويل:([^|\]]+)/)?.[1]?.trim() || '';
    if (!selected || genericLabels.has(selected)) return [];
    return [selected, ...selected.split(/\s+/)]
      .filter(name => name.length >= 2 && !genericLabels.has(name));
  });

  return [...new Set(names)].sort((a, b) => b.length - a.length).reduce((out, name) => {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const matcher = new RegExp(`(?<![\\p{L}\\p{N}])${escaped}(?![\\p{L}\\p{N}])`, 'giu');
    return out.replace(matcher, 'الفريق');
  }, String(text || ''));
}
...
function stripAvoidedContent(reply, config = {}) {
  ...
  const protectedMarkers = protectEscalationMarkers(cleaned);
  const privacySafe = replacePrivateContactNames(protectedMarkers.masked, protectedMarkers.markers);
  const { masked, urls } = protectUrls(privacySafe);
  cleaned = stripQuotes(masked);

  let removedForbidden = false;
  if (config?.replyStyle?.allowAutomationDisclosure !== true) {
    const identity = stripStructuralIdentityDisclosure(cleaned);
    cleaned = identity.text;
    removedForbidden = identity.removed;
  }
  const employeePersona = stripUnauthorizedEmployeePersona(cleaned, config);
  cleaned = employeePersona.text;
  removedForbidden = removedForbidden || employeePersona.removed;

  const phrases = effectiveAvoidList(config);
  for (const phrase of phrases) {
    const before = cleaned;
    cleaned = stripPhrase(cleaned, phrase);
    if (cleaned !== before) removedForbidden = true;
  }

  cleaned = stripListMarkers(cleaned);
  cleaned = tidyWhitespace(cleaned);
  cleaned = restoreUrls(cleaned, urls);
  cleaned = restoreEscalationMarkers(cleaned, protectedMarkers.markers);

  if (!cleaned || cleaned.length < 3) return removedForbidden ? '' : original.trim();
  return cleaned;
}
...
function ensureNaturalMultiline(text, words) {
  ...
  // A greeting often arrives on its own line before one long unpunctuated body.
  // Format each existing line independently instead of treating the first
  // newline as proof that the whole reply is already formatted. Empty segments
  // are retained so intentional blank lines remain intact.
  return input.split('\n').map((segment) => {
    const trimmed = segment.trim();
    if (!trimmed) return '';
    const bySentence = breakBySentence(trimmed);
    ...
    if (tokens.length > Math.max(6, words)) return breakByWords(trimmed, words);
    return trimmed;
  }).join('\n');
}
