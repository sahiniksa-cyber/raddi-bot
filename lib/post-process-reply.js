...
function tidyWhitespace(text) {
  return text
    .replace(/[ \t]+/g, ' ')
    // Normalize spacing after sentence punctuation…
    .replace(/ ?([،؟!]) ?/g, '$1 ')
    // …but a period BETWEEN digits is a decimal point (e.g. 207.9), never a
    // sentence end — do not split it. Only space a period NOT followed by a digit.
    .replace(/ ?\.(?!\d) ?/g, '. ')
    .replace(/\n{3,}/g, '\n\n')
    .replace(/\s+\./g, '.')
    .trim();
...
// كسر على حدود طبيعية فقط: أسطر الذكاء الموجودة + نهايات الجُمل (؟ ! …) +
// الفاصلة العربية (،). لا يكسر أبداً في منتصف عبارة (بعكس عدّ الكلمات الذي
// كان يقطع "والذكاء الاصطناعي" نصفين). آمن مع الأرقام (5.2) والروابط لأنها
// مقنّعة قبل هذه المرحلة، ولأننا نكسر فقط بعد علامة يتبعها مسافة.
function breakByNaturalBoundaries(text) {
  return String(text)
    .replace(/([.؟!…]+)[ \t]+/g, '$1\n')  // بعد نهاية الجملة
    .replace(/(،)[ \t]+/g, '$1\n')          // بعد الفاصلة العربية
    .split(/\n+/)
...
// تجميع المواضيع (حتمي): السطر الذي يبدأ بحرف ربط (و / أو / ثم / لكن / بس /
// عشان / مع / كمان / أيضاً / يعني / لأن) = تكملة للموضوع السابق فيُدمج معه؛
// وإلا فهو بداية موضوع جديد → كتلة مستقلة يفصلها فراغ. يعالج التفكّك (سطر فاضي
// بعد كل جملة) بجمع تفاصيل الموضوع الواحد سوا، والفراغ بين المواضيع فقط.
const CONTINUATION_START_RE = /^(?:و|أو|او|ثم|لكن|بس|عشان|علشان|مع|كمان|أيضا|ايضا|يعني|لأن|لان)(?![ء-ي])/u;
function breakByTopicBlocks(text, count) {
  // إذا رجّع الذكاء كتلة واحدة بلا أسطر (يتجاهل توجيه التنسيق أحياناً)، اصنع
  // الحدود الطبيعية أولاً (نهايات الجُمل + الفاصلة العربية) حتى توجد مواضيع
  // تُجمَّع؛ وإلا احترم أسطر الذكاء الموجودة كما هي (صفر تغيير على الرد المنسّق).
  const source = /\n/.test(String(text)) ? String(text) : breakByNaturalBoundaries(text);
  const lines = source.split(/\n+/).map(l => l.trim()).filter(Boolean);
  const blocks = [];
  for (const line of lines) {
    // "و" كبادئة ملتصقة (ومضمون، وتوصلك) = تكملة أيضاً
    const isContinuation = /^و[ء-ي]/u.test(line) || CONTINUATION_START_RE.test(line);
    if (isContinuation && blocks.length) blocks[blocks.length - 1] += ' ' + line;
    else blocks.push(line);
  }
  const gap = '\n'.repeat(Math.max(2, parseInt(count, 10) || 2)); // سطر فاضي بين الكتل على الأقل
...
function applyLineBreakFormat(text, config = {}) {
  ...
  if (mode === 'sentence') {
    ...
    if (process.env.CLEAN_LINE_BREAKS_ENABLED === 'true') {
      // Clean mode: GROUP a topic's lines together (continuation lines merge)
      // and put the merchant-chosen gap between DIFFERENT topics only — never
      // a blank line after every sentence.
      out = breakByTopicBlocks(masked, count);
    } else {
