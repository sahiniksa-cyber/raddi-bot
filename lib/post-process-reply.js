...
function stripPhrase(text, phrase) {
  ...
  // A multi-word merchant phrase must still match when the model inserts a
  // comma/question mark between its words. Single words retain strict word
  // boundaries so avoiding "فريق" cannot mangle "الفريق" into "ال".
  ...
  return lines.map(line => {
    ...
    while (from <= workingNormalized.length) {
      ...
      if (normalizedPhrase === '.' && /\d/.test(before || '') && /\d/.test(after || '')) {
        // A merchant banning sentence periods must not turn a decimal such as
        // "3.5" into the materially different value "35".
...
function protectEscalationMarkers(text) {
  const markers = [];
  const masked = String(text || '').replace(ESC_MARKER_TOKEN_RE, (marker) => {
    markers.push(marker);
...
function tidyWhitespace(text) {
  return text
    .replace(/[ \t]+/g, ' ')
    .replace(/ ?([،؟!]) ?/g, '$1 ')
    // Sentence periods get one following space, but decimal points stay intact.
    .replace(/ ?\.(?!\d) ?/g, '. ')
    .replace(/\n{3,}/g, '\n\n')
    .replace(/\s+\./g, '.')
    .trim();
...
// These are platform safety invariants, not merchant style preferences. A
// custom avoid-list must not accidentally re-enable direct technical identity
// disclosure in customer replies.
const PLATFORM_IDENTITY_DISCLOSURES = [
  'أنا ذكاء اصطناعي',
  'كذكاء اصطناعي',
  'أنا بوت',
  'أنا روبوت',
...
const STRUCTURAL_IDENTITY_DISCLOSURE_RES = [
  ...
  /(^|[^\p{L}\p{N}])(?:i\s*(?:am|['’]m)|as)\s+(?:an?\s+)?(?:(?:ai|artificial\s+intelligence)(?:\s+(?:assistant|system|program))?|bot|chatbot|language\s+model)(?=$|[^\p{L}\p{N}])/giu,
];
...
function replacePrivateContactNames(text, markers = []) {
  // Only scrub when an actual internal transfer marker exists. This keeps
  // ordinary customer-facing names intact while ensuring the selected
  // employee's full or short name cannot leak in a handoff acknowledgement.
  ...
  const names = markers.flatMap((marker) => {
    ...
    return [selected, ...selected.split(/\s+/)].filter(name => name.length >= 2 && !genericLabels.has(name));
  });

  return [...new Set(names)].sort((a, b) => b.length - a.length).reduce((out, name) => {
    const matcher = new RegExp(`(?<![\\p{L}\\p{N}])${escapeRegExp(name)}(?![\\p{L}\\p{N}])`, 'giu');
...
function stripAvoidedContent(reply, config = {}) {
  ...
  let removedForbidden = identity.removed;

  const replyStyle = config?.replyStyle || {};
  const configuredAvoided = [
    ...PLATFORM_IDENTITY_DISCLOSURES,
  ...
  // Structural cleanup may occasionally leave only a marker; preserve the old
  // fail-safe for that case. Forbidden content is different: restoring it here
  // would directly violate the merchant's "never say" setting.
