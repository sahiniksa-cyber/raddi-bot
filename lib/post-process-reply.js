...
function applyLineBreakFormat(text, config = {}) {
  ...
  if (mode === 'sentence') {
    ...
    // Clean mode (flag): break on natural boundaries — never mid-phrase — then
    // apply the merchant-chosen gap (count). Legacy path kept behind the flag.
    out = process.env.CLEAN_LINE_BREAKS_ENABLED === 'true'
      ? breakByNaturalBoundaries(masked)
      : ensureNaturalMultiline(masked, words);
