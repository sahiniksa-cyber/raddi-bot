...
//   topic     → the model separates topics with blank lines; we normalise the
//               gap to exactly `count` newlines (the model decides boundaries)
//   ai        → leave as written (the model distributes lines itself)
...
function normalizeTopicGaps(text, count) {
  // Collapse any run of 2+ newlines (a blank-line separator the model inserted
  // between topics) to exactly `count` newlines. Single newlines are left as-is.
  return String(text).replace(/(?:[ \t]*\n[ \t]*){2,}/g, '\n'.repeat(count));
}
...
function applyLineBreakFormat(text, config = {}) {
  ...
  if (mode === 'connected' || mode === 'ai') return text;
  ...
  let out = masked;
  if (mode === 'sentence') out = breakBySentence(masked);
  else if (mode === 'words') out = breakByWords(masked, words);
  else if (mode === 'topic') out = normalizeTopicGaps(masked, count);
