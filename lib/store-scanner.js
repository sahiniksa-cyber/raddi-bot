...
async function fetchWithPuppeteer(url, {
  ...
  const browserApi = puppeteer || require('puppeteer-core');
  const opts = {
    headless: true,
  ...
  let browser;
  try {
    browser = await browserApi.launch(opts);
...
async function scanStore(url, {
  ...
  zidManagerToken,
  logger,
  browserFetcher = fetchWithPuppeteer,
} = {}) {
  ...
  if (result.products.length < 3) {
    ...
    try {
      const puppeteerHtml = await browserFetcher(url);
