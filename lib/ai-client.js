...
const { buildSlaBreachBlock } = require('../src/services/instruction-routing/sla-breach');
const { resolveTenantTimezone, buildCurrentTimeBlock, withTimestampPrefix } = require('../src/services/ai/time-context');
const { buildCalculationBlock, resolvePriceComputation, buildPriceComputationBlock, deriveResolvedPricingContext } = require('../src/services/ai/deterministic-calc');
...
const { splitInstructionsForPrompt, redactInternalDestinations } = require('../src/services/ai/support-contract');
const {
...
class AIClient {
  ...
  async reviewBeforeSend({ draft, history = [], customerText = '', source = 'ai_reply' } = {}) {
    ...
    // The LLM reviewer may ADVISE suppression, but its word alone must not drop a
    // legitimately new reply (e.g. a correct deterministic price answer to a new
    // "كم؟"). Only the deterministic duplicate guard below — a real double-send /
    // full repeat with NO new customer turn — may actually suppress. When the
    // reviewer advised suppress (often returning an empty reply), fall back to the
    // draft so post-processing + the deterministic guard make the real decision.
    const baseReply = (reviewed.reply && String(reviewed.reply).trim()) ? reviewed.reply : draft;
    let reply = cleanupFinalReplyDeterministically(baseReply);
    ...
    return {
      ...reviewed,
      reply,
      // The deterministic guard did NOT suppress → this reply is sent. Override any
      // LLM-advised suppression flag/decision so the caller does not drop it.
      suppressed: false,
      audit: {
        ...reviewed.audit,
        decision: reviewed.audit?.decision === 'suppress' ? 'pass' : (reviewed.audit?.decision || 'pass'),
  ...
  async raw({ messages, temperature = 0.2, max_tokens = 600, response_format } = {}) {
    ...
    // PRIVACY: this is the SECOND provider entry point (auxiliary structured calls
    // like conversation-state extraction, whose input can carry LAST_BOT_REPLY —
    // possibly an old assistant reply that leaked the number). Sanitize a COPY of
    // every message before the provider call; never mutate the caller's array/config.
    const safeMessages = (Array.isArray(messages) ? messages : []).map((m) => ({
      ...m,
      content: typeof m?.content === 'string'
        ? redactInternalDestinations(m.content, this.config)
        : m?.content,
    }));
    const payload = { model: effModel, messages: safeMessages, temperature, max_tokens };
  ...
  buildSystemPrompt(history = [], opts = {}) {
    ...
    // PRIVACY: redact internal escalation destinations (incl. a legacy owner phone
    // the routing layer extracts from botInstructions) from the LLM-visible copy.
    // Stored config is untouched; server-side routing still sees the original.
    const customInstructions = redactInternalDestinations(String(this.config.botInstructions || '').trim(), this.config);
...
- لا تُصدر علامة تحويل [تحويل:...] جديدة لنفس الموضوع. لا تستخدمها إلا إذا طلب العميل صراحةً التحدث مع مسؤول الآن، أو أبلغ عن مشكلة جديدة مختلفة تماماً.`
    ...
    // Context Engine V2: the block also receives the latest customer text so it
    // can select the memories relevant to THIS turn (§13) rather than dumping all.
    const conversationStateCanInject = process.env.CONVERSATION_STATE_ENABLED === 'true'
      && opts.conversationStateCanInject === true;
    const conversationStateBlock = conversationStateCanInject
      ? buildConversationStateBlock(opts.conversationState, { canInject: true, latestUserText: lastUserText })
      : '';
    ...
    // SLA breach (deterministic, computed by the worker from a reliable system
    // timestamp): when a breach is proven this OVERRIDES the plain SLA block —
    // the bot must stop stating the window as still-upcoming. Empty otherwise.
    const slaBreachBlock = routingEnabled ? buildSlaBreachBlock(opts.slaBreach) : '';
    const routedPoliciesBlock = routingEnabled
      ...
      : '';
    // Deterministic pricing rule (Platform-level, tenant-driven). Empty unless
    // the tenant configured a rule → zero impact for tenants without one.
    const calcBlock = buildCalculationBlock(this.config);
    // Deterministic price ANSWER for this turn: resolve the referenced
    // product/variant from conversation context and actually run computePrice()
    // here, then hand the computed total to the model as a fact (or a single
    // clarifying question when ambiguous — never an escalation). Empty when the
    // turn is not a price question / nothing resolvable.
    // Context Engine → pricing bridge (§15/§16): when the state is current, pass
    // the resolved product/variant/payment-method so a short reply ("تمارا") then
    // "كم؟" computes deterministically even when the raw customer text at this turn
    // names neither. Falls back to regex resolution when no context is available.
    const resolvedPricingContext = conversationStateCanInject
      ? deriveResolvedPricingContext(opts.conversationState)
      : null;
    const priceComputationBlock = buildPriceComputationBlock(
      resolvePriceComputation({ history, latestUserText: lastUserText, config: this.config, resolvedContext: resolvedPricingContext }),
    );
    ...
    let escalationBlock = '';
    if (esc?.length > 0) {
      // PRIVACY (FIX 1): internal escalation DESTINATIONS (phone/target/jid/
      // groupJid/messageTemplate) are NEVER rendered into the model prompt. The
      // model only needs the contact name/role/condition to DECIDE an escalation;
      // the server resolves the real destination afterward. Exposing the number
      // here leaked the owner's personal phone to a customer (production incident).
...
- ممنوع منعاً باتاً أن تعطي العميل رقم جهة التصعيد أو أي رقم/واتساب/معرّف داخلي، وممنوع تطلب منه يتصل أو يراسل الجهة بنفسه. جهات التصعيد داخلية للنظام فقط ولا تُعرض للعميل إطلاقاً.
- كل ما عليك: ضع علامة التصعيد فقط، والنظام يتكفّل بتوصيلها للجهة الصحيحة داخلياً.
...
الرد: "تمام، سجّلت طلبك ورفعته للمختص. [تحويل:المالك|طلب التواصل بخصوص الطلب 12345]"`;
    }

    // Instruction Authority (§2/§10) — PLATFORM DEFAULT: a long botInstructions
    // must NEVER become the whole prompt base and dominate platform logic. It is
    // shadow-routed at runtime (using the PURE classifier, NO DB migration) into a
    // bounded, subordinate PERSONA block (style only) plus an authoritative
    // TENANT-FACTS block (products/policies/SLA/escalation/prohibitions the owner
    // wrote in free text) — so nothing the merchant wrote is lost, but the blob
    // stops being the dominating base. STYLE stays persona; UNKNOWN stays a
    // subordinate persona note (never promoted to platform authority).
    //
    // The ONLY way back to the legacy "blob-as-base" behavior is the explicit
    // kill-switch BOUNDED_BOT_INSTRUCTIONS_ENABLED=false (rollback), so correctness
    // never depends on a flag being switched ON.
    if (hasLongCustomInstructions && process.env.BOUNDED_BOT_INSTRUCTIONS_ENABLED === 'false') {
      return `${customInstructions}${knowledgeRules}${platformBlock}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${conversationStateBlock}${slaBlock}${slaBreachBlock}${routedPoliciesBlock}${calcBlock}${priceComputationBlock}${escalationBlock}${welcomeHint}`;
    }

    if (hasLongCustomInstructions) {
      const split = splitInstructionsForPrompt(customInstructions);
      const personaText = split.personaText;
      // Safety: only if the classifier produced NEITHER persona NOR facts do we
      // keep the raw text (as FACTS, never persona) so no content is ever lost.
      let factsText = split.factsText;
      if (!personaText && !factsText) factsText = customInstructions;
      const personaMax = parseInt(process.env.BOT_INSTRUCTIONS_MAX_CHARS || '1500', 10);
      // Blocker 4: an empty persona is VALID. When all instructions are
      // operational, do NOT re-inject the whole blob as persona — leave persona
      // empty; the operational content lives in the authoritative facts block.
      const persona = personaText.length > personaMax
        ? `${personaText.slice(0, personaMax)}…`
        : personaText;
      const personaBlock = persona
        ...
        : '';
      const legacyFactsBlock = factsText
        ? `\n\n<معلومات_وسياسات_التاجر>\n(مصدر حقيقة كتبه المالك: معلومات ومنتجات وسياسات وشروط تصعيد. التزم بها ولا تخترع ما ليس فيها. عند التعارض: المنتجات/الأسعار وحقول المنصة أدناه أعلى منها.)\n${factsText}\n</معلومات_وسياسات_التاجر>`
        : '';
...
${platformBlock}${legacyFactsBlock}${knowledgeRules}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${conversationStateBlock}${slaBlock}${slaBreachBlock}${routedPoliciesBlock}${calcBlock}${priceComputationBlock}${escalationBlock}${welcomeHint}`;
    }
...
📦 المنتجات المتوفرة:
${productsBlock}${productContext ? `\n\n📌 المنتجات المطابقة لسؤال العميل:\n${productContext}` : ''}
${platformBlock}${knowledgeRules}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${conversationStateBlock}${slaBlock}${slaBreachBlock}${routedPoliciesBlock}${calcBlock}${priceComputationBlock}${escalationBlock}${welcomeHint}`;
  }
  ...
  // Assemble the messages sent to the model: the system prompt (with a
  // tenant-timezone current-time block appended) followed by the history with
  // each turn prefixed by its clock time. PURE-ish (reads config + opts.now);
  // extracted so the time-context wiring is unit-testable without an API call.
  // Time is rendered ONLY here (the API copy) — the shared history objects stay
  // clean so dedup / state-extraction are unaffected. No time is ever invented.
  composeMessages(history, opts = {}) {
    const system = this.buildSystemPrompt(history, opts);
    const tz = resolveTenantTimezone(this.config);
    const now = opts.now instanceof Date ? opts.now : new Date();
    const timeBlock = buildCurrentTimeBlock({ now, timezone: tz });
    const systemWithTime = timeBlock ? `${system}${timeBlock}` : system;
    const renderedHistory = (Array.isArray(history) ? history : []).map((m) => ({
      role: m.role,
      content: withTimestampPrefix(m.content, m.ts, tz),
    }));
    const messages = [{ role: 'system', content: systemWithTime }, ...renderedHistory];
    // PRIVACY — FINAL catch-all: sanitize EVERY message content (system + history)
    // sent to the model, so a protected internal destination cannot leak through
    // ANY field (escalationContacts.when, escalationConditions, storeDescription,
    // policies…) or through prior conversation history (e.g. an already-leaked
    // assistant turn). Defense-in-depth over the earlier per-field redactions.
    // Only the LLM-visible copy is redacted — stored config/history are untouched.
    return messages.map((m) => ({
      role: m.role,
      content: redactInternalDestinations(m.content, this.config),
    }));
  }
  ...
  async getReply(history, opts = {}) {
    const { openai, model } = this.buildClient();
    const messages = this.composeMessages(history, opts);
