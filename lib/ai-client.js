...
${platformBlock}${knowledgeRules}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${escalationBlock}${welcomeHint}`;
  ...
  async getReply(history, opts = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        // escalation marker from being split. connected/ai are no-ops.
  ...
  async planConfigEdit(config, request) {
    ...
    const system = [
      ...
      'الأقسام (target): "products" أو "instant_replies" أو "do_not_reply" أو "prompt" (لو تعليمات عامة للبوت وليست بيانات منظّمة).',
