...
class AIClient {
   ...
   * extraction). Reuses the tenant's configured provider/model from
   * buildClient(); CONVERSATION_STATE_MODEL may override the model string when
   * the operator wants a cheaper one that the same client can route.
   */
...
- لا تُصدر علامة تحويل [تحويل:...] جديدة لنفس الموضوع. لا تستخدمها إلا إذا طلب العميل صراحةً التحدث مع مسؤول الآن، أو أبلغ عن مشكلة جديدة مختلفة تماماً.`
    ...
    // Generic conversation-state block (phase 1). Fail-soft: only injected when
    // the flag is on AND the caller confirms the stored state is current
    // (extraction_ok + reflects the messages being answered). A stale/failed
    // state is never presented as truth — the prompt falls back to transcript-
    // only behavior. Flag off ⇒ empty string ⇒ byte-identical legacy prompt.
    ...
    // SLA policies captured by the Instruction Routing Layer, rendered as
    // authoritative time facts (flagged). Empty string when off/none.
    const slaBlock = (process.env.INSTRUCTION_ROUTING_ENABLED === 'true')
      ? buildSlaBlock(this.config.slaPolicies)
      : '';
...
الرد: "تمام، بسجل طلبك ويتواصل معك المختص خلال وقت قصير. [تحويل:المالك|طلب التواصل بخصوص الطلب 12345]"`;
    ...
    // Bounded persona (Instruction Routing Layer, flagged). A long merchant
    // botInstructions must NOT become the entire prompt base and dominate the
    // platform's operational logic. When enabled, the merchant text is injected
    // as a capped, clearly-subordinate persona/style section inside the
    // structured prompt — the platform blocks (products/rules/escalation/state)
    // remain the authoritative source. Flag off ⇒ legacy behavior below.
...
${platformBlock}${knowledgeRules}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${conversationStateBlock}${slaBlock}${escalationBlock}${welcomeHint}`;
    }

    // Custom instructions (long) — owner provides the full behavior brief
    if (hasLongCustomInstructions) {
      return `${customInstructions}${knowledgeRules}${platformBlock}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${conversationStateBlock}${slaBlock}${escalationBlock}${welcomeHint}`;
    }
