...
class AIClient {
  ...
  async reviewBeforeSend({ draft, history = [], customerText = '', source = 'ai_reply' } = {}) {
    ...
    // Merchant post-processing can remove the only new clause. Suppress the
    // resulting repeat before grounding can replace it with a fresh handoff.
    const postProcessDuplicate = deterministicDuplicateGuard(reply, history);
    if (postProcessDuplicate.suppress) {
      return {
        reply: '',
        suppressed: true,
        audit: {
          ...
          repeatedClaims: postProcessDuplicate.repeatedClaims,
          violations: [
            ...(reviewed.audit?.violations || []),
            'semantic_duplicate_after_post_process',
          ],
        },
      };
    }
    ...
    return {
      ...
      audit: {
        ...reviewed.audit,
        // Preserve the first review's evidence. A second deterministic pass may
        // see the already-safe handoff text and otherwise erase the claim that
        // caused the fallback, making production diagnosis misleading.
        unsupportedClaims: [...new Set([
          ...(reviewed.audit?.unsupportedClaims || []),
          ...grounded.issues.map(issue => issue.value),
        ].filter(Boolean))],
...
- فرّق بين السؤال أو الطلب المفتوح وبين الملاحظة أو الاعتراض أو إنهاء الحديث. ذكر العميل لميزانيته أو مدة يتمناها لا يعني وحده أنه يسأل عن توفرها أو يطلب إنشاء عرض جديد.
- إذا قال العميل إن السعر غالٍ أو لا يناسبه ثم أنهى كلامه بالشكر بدون سؤال أو طلب مفتوح، فهذا اعتراض على السعر وليس نقص معلومات: لا تخترع سعراً أو خياراً، ولا تصعّد، ولا تعِد فتح البيع. اعترف بشعوره واختم برد قصير طبيعي ومتنوّع يناسب أسلوب المتجر.
- إذا طلب فعلاً خياراً أرخص أو سأل هل يمكن توفير مدة أو سعر محدد، فهذا طلب مفتوح: أجب فقط من معلومات المتجر، واسأل سؤالاً توضيحياً عند الحاجة بدل تحويله تلقائياً.
...
- استخدم المعرفة العامة في الحديث العادي، واسمح باستنتاج طبيعي منخفض المخاطر ملازم لنوع المنتج أو الخدمة إذا لم ينشئ وعداً تجارياً مؤثراً. مثال: الغسيل الخارجي للسيارة يشمل تنظيف المرايا الخارجية عادةً حتى لو لم تُذكر المرايا حرفياً؛ جاوب مباشرة ولا تصعّد.
- لا تستنتج أبداً من طبيعة المنتج حقائق تجارية أو تعاقدية قد تغيّر قرار الشراء: السعر، الخصم، التوفر، المدة، الضمان، التوصيل، التوافق، الاسترجاع، الحالة المالية، التغطية، طبقة نانو أو أي خدمة إضافية. هذه يجب أن تكون موجودة في مصادر المتجر.
- عدم وجود جملة مطابقة حرفياً في البرومنت لا يجعل الاستنتاج الطبيعي منخفض المخاطر معلومة مفقودة. افهم طبيعة المنتج أولاً، ثم فرّق بين ما هو ملازم وبديهي وبين وعد إضافي يحتاج مصدراً.
- إذا سأل العميل عن حقيقة تجارية مؤثرة خاصة بالمنتج وغير موجودة في جميع المصادر، استخدم البديل الآمن: "ودّي أأكد لك المعلومة من المختص قبل ما أعطيك جواب مؤكد، تسمح لي؟" — ثم استخدم تصعيداً إذا كان متاحاً، أو اطلب من العميل التواصل المباشر.
...
3. سؤال عن حقيقة تجارية مؤثرة خاصة بالمنتج لا تجد لها إجابة في أي حقل من التعليمات أو المنتجات بعد محاولة جدية. لا تصعّد الحديث العام أو الاستنتاج الطبيعي منخفض المخاطر الملازم للخدمة؛ مثال غسيل المرايا الخارجية ضمن الغسيل الخارجي للسيارة.
...
الرد: "تمام، بسجل طلبك ويتواصل معك المختص خلال وقت قصير. [تحويل:المالك|طلب التواصل بخصوص الطلب 12345]"`;
    ...
    if (hasLongCustomInstructions) {
      return `${customInstructions}${knowledgeRules}${platformBlock}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${escalationBlock}${welcomeHint}`;
    }
...
${platformBlock}${knowledgeRules}${policyBlock}${instantBlock}${profileBlock}${pendingEscalationBlock}${escalationBlock}${welcomeHint}`;
  }
        ...
        if (qualityGateEnabled) {
          try {
            ...
            qualityGateAudit = {
              ...
              hardFallback: true,
              confidence: 0,
              requiresHuman: grounded.usedFallback,
              humanReason: grounded.usedFallback ? 'unsupported_information' : '',
              handoffSummary: grounded.usedFallback ? lastUserText.slice(0, 120) : '',
              latencyMs: 0,
  ...
  async proposePromptEdit(currentInstructions, editRequest) {
    ...
    const system = [
      ...
      'السطر الأول: ملخص سطر واحد بالعربي لما تغيّر.',
