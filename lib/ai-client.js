...
الرد: "تمام، بسجل طلبك ويتواصل معك المختص خلال وقت قصير. [تحويل:المالك|طلب التواصل بخصوص الطلب 12345]"`;
    ...
    if (hasLongCustomInstructions) {
      const { personaText, factsText } = splitInstructionsForPrompt(customInstructions);
      const personaMax = parseInt(process.env.BOT_INSTRUCTIONS_MAX_CHARS || '1500', 10);
      const personaSource = personaText || customInstructions; // never end up with an empty persona
      const persona = personaSource.length > personaMax
        ? `${personaSource.slice(0, personaMax)}…`
        : personaSource;
