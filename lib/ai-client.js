...
} = require('../src/services/ai/canonical-prompt-context');
...
class AIClient {
  ...
  buildSystemPrompt(history = [], opts = {}) {
    const latestCustomerText = opts.latestUserText
      || [...history].reverse().find(message => message.role === 'user')?.content
      || '';
    return buildCanonicalAutomatedPrompt({
      config: this.config,
      customerText: latestCustomerText,
      alreadyAnswered: opts.instantAnswered,
      customerProfile: opts.customerProfile,
      escalationPending: opts.escalationPending === true,
    });
    ...
    const hasLongCustomInstructions = false;
