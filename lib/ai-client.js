...
const { stripProductSections } = require('../src/services/products/product-knowledge');
const {
...
function formatScopedProducts(scoped = {}) {
  if (!Array.isArray(scoped.products) || scoped.products.length === 0) {
    return 'المنتج غير محدد. اطلب اسم المنتج باختصار قبل ذكر أي سعر أو مدة.';
  }
  return scoped.products.map((product, index) => {
    const standalonePlan = product.plans.length === 1
      && !product.plans[0].label
      && !product.plans[0].duration
      && product.plans[0].price
      ? product.plans[0]
      : null;
    const standalonePrice = standalonePlan
      ? `${standalonePlan.price.amount} ${standalonePlan.price.currency === 'SAR' ? 'ريال' : standalonePlan.price.currency}`
      : '';
    const lines = [`${index + 1}. ${product.canonicalName}${standalonePrice ? ` — ${standalonePrice}` : ''}`];
    if (product.description) lines.push(`   ${product.description}`);
    if (product.longDescription) lines.push(`   ${product.longDescription}`);
    if (product.url) lines.push(`   الرابط: ${product.url}`);
    for (const plan of product.plans) {
      if (!plan.available || plan === standalonePlan) continue;
      const price = plan.price ? `${plan.price.amount} ${plan.price.currency === 'SAR' ? 'ريال' : plan.price.currency}` : 'السعر غير محدد';
      lines.push(`   • ${plan.label || 'الخيار'}: ${price}`);
    }
    return lines.join('\n');
  }).join('\n');
}
...
class AIClient {
  ...
  async reviewBeforeSend({ draft, history = [], customerText = '', source = 'ai_reply' } = {}) {
    ...
    const finalValidation = finalizeReply({
      draft: reply,
      history,
      customerText,
      config: this.config,
      catalogVersion: this.config.productCatalogVersion || 0,
      reviewerAudit: reviewed.audit,
    });
    if (finalValidation.decision !== 'validated') {
      const error = new Error(`pre-send final validation blocked: ${finalValidation.reason}`);
      error.code = 'FINAL_REPLY_VALIDATION_BLOCKED';
      error.validation = finalValidation;
      throw error;
    }
    reply = finalValidation.reply;
    ...
    return {
      ...
      audit: {
        ...
        finalValidation: finalValidation.audit,
      },
    };
  }
  ...
  buildSystemPrompt(history = [], opts = {}) {
    ...
    const employeeName = r.employeeNameEnabled === true && String(r.employeeName || '').trim()
      ...
      : 'موظف خدمة العملاء';
    const avoid = effectiveAvoidList(this.config).join('، ');
    const customInstructions = stripProductSections(this.config.botInstructions);
    ...
    const productCatalog = buildProductFactCatalog(this.config, {
      catalogVersion: this.config.productCatalogVersion || 0,
    });
    ...
    const scopedProducts = buildScopedProductContext({
      catalog: productCatalog,
      focus: productFocus,
    });
    const productContext = productFocus.status === 'resolved'
      ? `المنتج المحدد: ${scopedProducts.products.map(product => product.canonicalName).join('، ')}`
      : 'المنتج غير محدد؛ لا تذكر سعراً أو مدة واطلب اسم المنتج.';
    ...
    const productsBlock = formatScopedProducts(scopedProducts);
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const finalValidation = finalizeReply({
          ...
          config: this.config,
          catalogVersion: this.config.productCatalogVersion || 0,
          reviewerAudit: qualityGateAudit,
        });
        if (finalValidation.decision !== 'validated') {
          const error = new Error(`final reply validation blocked: ${finalValidation.reason}`);
          error.code = 'FINAL_REPLY_VALIDATION_BLOCKED';
          error.validation = finalValidation;
          throw error;
        }
        ...
        this.lastDebug.finalValidation = finalValidation;
