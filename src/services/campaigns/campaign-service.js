...
function normalizeAudienceRules(value = {}) {
  ...
  const source = ['smart', 'keywords', 'contacts', 'conversations', 'all'].includes(input.source) ? input.source : 'smart';
  ...
  if (source === 'smart' && hasExplicitStates && states.length === 0) throw badRequest('اختر تصنيفاً واحداً على الأقل للتحديد الذكي');
...
async function resolveAudience(database, userId, rules = {}) {
  const source = rules.source || 'smart';
  const rows = [];
  if (source === 'smart') {
    const { clauses, params } = buildAudienceWhere(userId, rules);
    const result = await database.query(
      `SELECT s.conversation_id, s.sender, c.phone_number AS normalized_phone,
              COALESCE(c.metadata->>'name', '') AS customer_name,
              s.product_key, s.product_name, s.state AS customer_state,
              s.confidence, s.evidence_message_id, s.evidence_text, s.source
  ...
  async function importContacts(userId, buffer, originalName = '') {
    ...
    for (const sheet of workbook.worksheets) {
      ...
      sheet.eachRow((row, number) => {
        if (number === 1) return;
        const data = {};
        row.eachCell({ includeEmpty: true }, (cell, col) => { data[headers[col] || `column ${col}`] = cell.text; });
    ...
    const productHeaders = ['product', 'product name', 'المنتج', 'اسم المنتج'];
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      await database.query(
        `INSERT INTO campaign_contacts (user_id, normalized_phone, sender, name, source, metadata)
         VALUES ($1, $2, $3, $4, 'import', $5::jsonb)
         ON CONFLICT (user_id, normalized_phone) DO UPDATE SET name = COALESCE(EXCLUDED.name, campaign_contacts.name),
           source = 'import', metadata = campaign_contacts.metadata || EXCLUDED.metadata, updated_at = NOW()`,
        [userId, phone, senderFromPhone(phone), name, JSON.stringify({ file: originalName, sheet: sheetName, row: rowNumber })],
      );
      if (productName) {
        const key = normalizeProductText(productName);
        await upsertSignals({
          database, userId, sender: senderFromPhone(phone), signals: [{
            productKey: key,
            productName,
            state: orderReference ? 'ordered_confirmed' : (importedState || 'needs_verification'),
            confidence: orderReference || importedState ? 1 : 0.6,
            orderReference: orderReference || null,
            evidenceText: orderReference
              ? `Imported order reference: ${orderReference}`
              : (importedState ? `Imported merchant classification: ${importedState}` : 'Imported product association without verified order reference'),
            source: importedState ? 'merchant_file_import' : 'contact_import',
            metadata: { file: originalName, sheet: sheetName, row: rowNumber, importedState },
          }],
        });
      }
      added += 1;
    }
    return { added, invalid, totalRows: rows.length };
  }
  ...
  async function listSignals(userId, filters = {}) {
    ...
    const result = await database.query(
      `SELECT s.id AS signal_id, s.conversation_id, s.sender,
              ...
              s.order_reference, s.source, s.last_detected_at, s.metadata
       FROM customer_product_signals s
       LEFT JOIN conversations c ON c.id = s.conversation_id
