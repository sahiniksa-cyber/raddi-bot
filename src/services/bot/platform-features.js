...
function buildPlatformPromptBlock(config = {}, {
  ...
  // أسلوب الكتابة: الافتراضي (FALSE) = جُمل متصلة بدون تنسيق (سلوك حالي بدون تغيير).
  // عند تفعيل multilineFormat (TRUE) = وزّع الرد على عدة أسطر قصيرة بشكل بشري.
  const multiline = r.multilineFormat === true;
  const writingStyleLine = multiline
    ? '- 🔑 تنسيق الرد (قاعدة ملزمة): وزّع ردك على عدة أسطر قصيرة، اجعل كل فكرة في سطر مستقل مع سطر فاصل بين الأفكار عند المناسبة — لا ترسله ككتلة نصية واحدة. اكتبه كأنه رسائل إنسان طبيعية على واتساب. يبقى ممنوع التعداد النقطي بالرموز (•، -، *) وترقيم القوائم وعناوين الماركداون.'
