...
const { resolveLineBreakSettings } = require('../../../lib/post-process-reply');
...
function buildPlatformPromptBlock(config = {}, {
  ...
  const escalationConditions = String(config.escalationConditions || '').trim();
  const escalationLine = escalationConditions
    ? `\n- 🔔 صعّد للمختص (بكتابة علامة التحويل) في هذه الحالات أيضاً، إضافةً لعدم معرفتك للجواب: ${escalationConditions}`
    : '';

  // أسلوب الكتابة (تنسيق الأسطر): يختاره التاجر من الداشبورد (lineBreakMode).
  // التوجيه هنا للبوت؛ الأوضاع الميكانيكية (sentence/words) تُفرض أيضاً بالكود
  // عبر applyLineBreakFormat بعد توليد الرد. التوافق الخلفي: multilineFormat=true → ai.
  const NO_FORMAT = 'ممنوع التعداد النقطي بالرموز (•، -، *) وترقيم القوائم وعناوين الماركداون.';
  const { mode: lineBreakMode } = resolveLineBreakSettings(r);
  const writingStyleByMode = {
    connected: `- أسلوب الكتابة رسائل واتساب طبيعية بجُمل متصلة: ${NO_FORMAT} إلا إذا طلب المالك ذلك صراحةً في تعليماته.`,
    sentence: `- 🔑 تنسيق الرد (قاعدة ملزمة): اكتب كل جملة في سطر مستقل (سطر جديد بعد كل جملة). ${NO_FORMAT}`,
    topic: `- 🔑 تنسيق الرد (قاعدة ملزمة): افصل بين كل موضوع/فكرة والتي تليها بسطر فارغ (سطر فاصل)، واكتبه كرسائل واتساب طبيعية. ${NO_FORMAT}`,
    words: `- 🔑 تنسيق الرد (قاعدة ملزمة): وزّع ردك على أسطر قصيرة متتابعة بدل كتلة نصية واحدة. ${NO_FORMAT}`,
    ai: `- 🔑 تنسيق الرد (قاعدة ملزمة): وزّع ردك على عدة أسطر قصيرة، اجعل كل فكرة في سطر مستقل مع سطر فاصل بين الأفكار عند المناسبة — لا ترسله ككتلة نصية واحدة. اكتبه كأنه رسائل إنسان طبيعية على واتساب. يبقى ${NO_FORMAT}`,
  };
  const writingStyleLine = writingStyleByMode[lineBreakMode] || writingStyleByMode.connected;

  return `\n\n---\n📋 إعدادات المنصة الملزمة (استخدمها كلها مع تعليمات المالك، وليست مجرد معلومات ثانوية):\n- اسم المتجر: ${config.storeName || 'المتجر'}\n- وصف المتجر: ${config.storeDescription || '—'}\n- ساعات العمل: ${config.workingHours || '—'}\n- اسم الموظف/الشخصية: ${employeeName}\n- نبرة الرد: ${tone}\n- لغة الرد: ${language}\n- 🔑 طول الرد (قاعدة صارمة التزم بها حرفياً في كل رد): ${replyLength}\n- معدل الإيموجي: ${emoji}\n- عبارات ترحيب ممكنة: ${greetings}\n- عبارات إنهاء ممكنة: ${closings}\n- كلمات أو عبارات ممنوعة: ${forbidden}${escalationLine}\n- المنتجات الموجودة في المنصة:\n${productsBlock || '—'}${productContext ? `\n- المنتجات المطابقة لسؤال العميل:\n${productContext}` : ''}\n- عند التعارض: معلومات المنتجات والأسعار وحقول المنصة وتعليمات المالك هي مصدر الحقيقة. لا تعتمد على الذاكرة العامة ولا تخترع معلومة ناقصة.\n- طبّق هذه الخيارات في كل رد: اللغة، اللهجة، طول الرد، الإيموجي، النبرة، الترحيب، والخاتمة المناسبة.\n- تعليمات المالك ووصف المتجر أعلى أولوية مطلقة: اتبعها حرفياً في الأسلوب والتنسيق والمحتوى، وإذا منع المالك شيئاً (مثل النقاط أو عبارات معيّنة) فلا تستخدمه إطلاقاً.\n${writingStyleLine}`;
}
